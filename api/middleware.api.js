...
export async function getDashboardData() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      const [networkRes, poolsRes, nodesRes, runeSupplyRes, blockHeightRes] = await Promise.all([
        $axiosInstace.get(endpoints[process.env.NETWORK].THORNODE_URL + 'thorchain/network'),
        $axiosInstace.get(endpoints[process.env.NETWORK].THORNODE_URL + 'thorchain/pools'),
        $axiosInstace.get(endpoints[process.env.NETWORK].THORNODE_URL + 'thorchain/nodes'),
        $axiosInstace.get(endpoints[process.env.NETWORK].THORNODE_URL + 'thorchain/supply/rune'),
        $axiosInstace.get(endpoints[process.env.NETWORK].THORNODE_URL + 'thorchain/lastblock/thorchain')
      ])
      ...
      const totalPoolDepth = activePools.reduce((sum, pool) => sum + parseInt(pool.balance_rune || 0), 0)
      ...
      return Promise.reject(new Error('Failed to construct dashboard data from THORNode APIs'))
    }
  }
...
export async function getDashboardPlots() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('Failed to construct dashboard plots from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getExraNodesInfo() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('Failed to fetch extra nodes info from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getSaversInfo() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      const saversRes = await $axiosInstace.get(endpoints[process.env.NETWORK].THORNODE_URL + 'thorchain/savers')
      return { data: saversRes.data }
    } catch (error) {
      console.error('Error fetching savers from THORNode:', error)
      return Promise.reject(new Error('Failed to fetch savers from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getChainsHeight() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      const blockRes = await $axiosInstace.get(endpoints[process.env.NETWORK].THORNODE_URL + 'thorchain/lastblock')
      ...
      return Promise.reject(new Error('Failed to fetch chains height from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getHolders(asset = 'THOR.RUNE') {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('Holders data not available from THORNode API'))
    }
  }
...
export async function getPoolsHistory(period = '') {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('Failed to construct pools history from THORNode APIs'))
    }
  }
  if (period === 'day') {
    period = ''
...
export async function getOldPoolsHistory(period = '') {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      return { data: [] }
    } catch (error) {
      console.error('Error constructing old pools history from THORNode:', error)
      return Promise.reject(new Error('Old pools history not available from THORNode API'))
    }
  }
  if (period === 'day') {
    period = ''
...
export async function getServerTx(txid) {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('Failed to fetch transaction from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getRunePoolsInfo() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('Failed to fetch rune pools from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getOldRunePools() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('Old rune pools not available from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getOldRunePoolProvidersInfo() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      console.error('Error constructing old rune pool providers from THORNode:', error)
      return Promise.reject(new Error('Old rune pool providers not available from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getRunePoolProvidersInfo() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      const providersRes = await $axiosInstace.get(endpoints[process.env.NETWORK].THORNODE_URL + 'thorchain/rune_providers')
      return { data: providersRes.data }
    } catch (error) {
      console.error('Error fetching rune pool providers from THORNode:', error)
      return Promise.reject(new Error('Failed to fetch rune pool providers from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getBorrowersInfo() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      const borrowersRes = await $axiosInstace.get(endpoints[process.env.NETWORK].THORNODE_URL + 'thorchain/borrowers')
      return { data: borrowersRes.data }
    } catch (error) {
      console.error('Error fetching borrowers from THORNode:', error)
      return Promise.reject(new Error('Failed to fetch borrowers from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getSwapsWeekly() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('Weekly swaps data not available from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getStatsDaily() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('Failed to construct daily stats from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getFeesRewardsMonthly() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('Failed to construct fees rewards from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getAffiliateSwapsByWallet() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      console.error('Error constructing affiliate swaps by wallet from THORNode:', error)
      return Promise.reject(new Error('Affiliate swaps by wallet not available from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getAffiliateSwapsWeekly() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      console.error('Error constructing affiliate swaps weekly from THORNode:', error)
      return Promise.reject(new Error('Affiliate swaps weekly not available from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getAffiliateSwapsDaily() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      console.error('Error constructing affiliate swaps daily from THORNode:', error)
      return Promise.reject(new Error('Affiliate swaps daily not available from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getNodeOverview() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('Failed to construct node overview from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getAffiliateDaily() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('Affiliate daily not available from THORNode API'))
    }
  }
  return $axiosInstace.get(
  ...
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      if (params.txid) {
        try {
          const txRes = await $axiosInstace.get(
            endpoints[process.env.NETWORK].THORNODE_URL + `thorchain/tx/${params.txid}`
          )
          const tx = txRes.data
      ...
      return Promise.reject(new Error('Failed to construct actions from THORNode APIs'))
    }
  }
...
export async function getCoinMarketInfo() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('Coin market info not available from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getNodesInfo() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('Failed to fetch nodes info from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getTopSwaps() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('Top swaps not available from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getTopSwapsWeekly() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('Top swaps weekly not available from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getTopSwapsMonthly() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      console.error('Error constructing top swaps monthly from THORNode:', error)
      return Promise.reject(new Error('Top swaps monthly not available from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getNodes() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('Failed to fetch nodes from THORNode API'))
    }
  }
...
export async function getNetworkAllocation() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      console.error('Error constructing network allocation from THORNode:', error)
      return Promise.reject(new Error('Failed to construct network allocation from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getReserveHistory() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('Failed to construct reserve history from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getVotes(period = '30d') {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      const mimirRes = await $axiosInstace.get(endpoints[process.env.NETWORK].THORNODE_URL + 'thorchain/mimir')
      ...
      return Promise.reject(new Error('Failed to construct votes from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getBurnedBlocks() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('Burned blocks not available from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getInfraRUJIMerge() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('RUJI merge not available from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getAffiliateHistory(params) {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      console.error('Error constructing affiliate history from THORNode:', error)
      return Promise.reject(new Error('Affiliate history not available from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getTcyInfo(params) {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      ...
      return Promise.reject(new Error('TCY info not available from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getDenoms() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      const assetsRes = await $axiosInstace.get(endpoints[process.env.NETWORK].THORNODE_URL + 'thorchain/pool/assets')
      return { data: assetsRes.data }
    } catch (error) {
      console.error('Error fetching denoms from THORNode:', error)
      return Promise.reject(new Error('Failed to fetch denoms from THORNode API'))
    }
  }
  return $axiosInstace.get(
...
export async function getContracts() {
  if (!endpoints[process.env.NETWORK].SERVER_URL) {
    try {
      const inboundRes = await $axiosInstace.get(endpoints[process.env.NETWORK].THORNODE_URL + 'thorchain/inbound_addresses')
      return { data: inboundRes.data }
    } catch (error) {
      console.error('Error fetching contracts from THORNode:', error)
      return Promise.reject(new Error('Failed to fetch contracts from THORNode API'))
    }
  }
  return $axiosInstace.get(
